...
const breadcrumb = ref({ page:{id: 1, name: 'Configurações', url : "/configuracao"}, link:{id: 1, name: 'Campanhas', url : "/campanha"}});
...
watch(filter,
    throttle( ()=>{
        ...
        let queryRoute = route('campaign.index', Object.keys(query).length ? query : {
            remember: 'forget',
        });
...
function destroy(id) {
    if (confirm("Confirma a exclusão desta campanha?")) {
        form.delete(route('campaign.destroy', id));
    }
}
</script>
            ...
            <div class="bg-white">
                ...
                >   
                        ...
                        <tbody>
                            ...
                            >
                                <td
                                    ...
                                    class="px-4 py-4"
                                >
                                    <input type="checkbox" :id="`chk_${campaign.id}`" v-model="checked" />
                                </td>
                                <td
                                    scope="row"
                                    class="px-4 w-full"
                    ...
                    <div class="w-full bg-white justify-end py-5">
                        <Pagination :links="campaigns.links"></Pagination>
                    </div>
                </div>
            </div>
        </div><br><br>
